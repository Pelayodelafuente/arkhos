...
export function ExpenseCalendarCell({
  ...
  const cellClasses = [
    ...
    isSelected ? 'bg-[rgba(196,112,74,0.10)] ring-2 ring-inset ring-[var(--accent-terracotta)] border border-transparent' : '',
          ...
          {subscriptions.slice(0, 3).map((sub) => (
            <div
              ...
              className={`group/badge relative rounded-full ring-[1.5px] ${
                sub.cycle === 'annual' ? 'ring-[var(--module-notas)]' : 'ring-[var(--module-gastos)]'
              }`}
            >
              <ServiceAvatar
                name={sub.name}
