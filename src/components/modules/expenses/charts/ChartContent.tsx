        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} barCategoryGap="20%">
            ...
            <Bar dataKey="total" radius={[6, 6, 0, 0]}>
              {data.map((entry, i) => (
                <Cell key={i} fill={entry.isCurrent ? 'var(--module-gastos)' : 'rgba(59,120,176,0.35)'} />
              ))}
            </Bar>
            <Bar dataKey="prevTotal" radius={[6, 6, 0, 0]} fillOpacity={0}>
              {data.map((_, i) => (
                <Cell key={i} fill="transparent" stroke="var(--module-gastos)" strokeWidth={1} strokeDasharray="4 4" />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
