// ══════════════════════════════════════
// Arkhos — Dashboard Data Layer
// Módulo Dashboard: agregado server-side de los 6 módulos para la home
// ══════════════════════════════════════
...
import type {
  ...
  SnapshotData,
  SubscriptionData,
} from '@/components/modules/dashboard/dashboard-view'
...
/**
 * Agrega todos los datos que necesita `DashboardView`: actividad, proyectos,
 * snapshots de patrimonio, suscripciones, plataformas (con valores agregados
 * de Indexa/Horos/Mintos/Crypto), notas recientes y datos de mercado
 * (CoinGecko, Fear & Greed, cache de métricas de Mercados, watchlist ETFs).
 *
 * Relocalización literal de la lógica que vivía en `(dashboard)/page.tsx`.
 */
export async function getDashboardData(supabase: Client, userId: string): Promise<DashboardData> {
  const [
    ...
    Promise.all([
      fetchWithTimeout(
        ...
        .then((r): Promise<CoinGeckoResponse | null> => (r.ok ? r.json() : Promise.resolve(null)))
        .catch(() => null),
      fetchWithTimeout('https://api.alternative.me/fng/?limit=1', {
        next: { revalidate: 3600 },
      })
        .then((r): Promise<FearGreedResponse | null> => (r.ok ? r.json() : Promise.resolve(null)))
        .catch(() => null),
    ]),
  ])
  ...
  const marketData: MarketData = {
    ...
    fearGreed: fng ? { value: Number(fng.value), label: fng.value_classification } : null,
