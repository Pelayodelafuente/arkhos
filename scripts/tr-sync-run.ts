...
if (TR_COOKIE_FILE_B64) {
  ...
  console.warn('⚠️  TR_COOKIE_FILE_B64 no configurado — se intentará login con OTP (fallará en CI)')
}
...
console.log(`[${new Date().toISOString()}] Sync iniciado (log: ${syncLogId})`)
...
try {
  ...
  if (!loginOk) throw new Error('Login fallido — sesión expirada, ejecutar tr-auth.ts localmente')
  ...
  await Promise.allSettled(
    allIsins.map(async (isin) => {
      try {
        // TR tickers use format: ISIN.EXCHANGE (LSX = London Stock Exchange, often default for ETFs)
        const ticker = await subscribeOnce<TRTickerResponse>(api, 'ticker', { id: `${isin}.LSX` })
        const price = parseFloat(ticker.last?.price ?? '0')
        if (price > 0) tickerPrices.set(isin, price)
      } catch {
        // Ticker not available on LSX — skip, price won't be updated for this ISIN
      }
    })
  )
